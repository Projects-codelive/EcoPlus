...
export const CarbonGauge = ({ value, maxValue }: CarbonGaugeProps) => {
  ...
  const getColor = useMemo(() => {
    if (percentage <= 33) return 'hsl(142 71% 45%)'; // Green (Tailwind green-600 approx)
    if (percentage <= 66) return 'hsl(24 94% 50%)'; // Orange (Tailwind orange-500 approx)
    return 'hsl(0 84% 60%)'; // Red (Tailwind destuctive/red-500 approx)
  }, [percentage]);
